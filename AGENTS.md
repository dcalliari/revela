...
- Demo/test without a camera: `REVELA_DEMO=1` only (env — never compile-time config alone, never a Host toggle). Fake presence, never spawns gphoto2, Host badge + **Disparar (demo)** / `D` writes a synthetic JPEG into the editorial folder for the real inotify→ingest path. See README and `CameraServer.demo_fire/1`.
