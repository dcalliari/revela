...
- After a successful web preview, `Revela.Capture.Ingest` discards the camera JPEG by default (keeps RAW + preview; clears `original_path`). Opt out with `REVELA_KEEP_CAMERA_JPEG=1` / `config :revela, :keep_camera_jpeg, true` (see README). Disk shot estimates use the remaining on-disk files per stem and a RAW-only fallback when discard is on.
