...
Keyboard shortcuts (`1`–`5`, `0`/Backspace/Delete, arrows, `L`) are handled in
those LiveViews; `ViewerComponents` renders no visual legend or overlay numbers
for them, only `aria-label`s per button (removed by captain decision, see
BACKLOG item 3). Do not reintroduce a special-case that only sets follow on
pick of the penultimate photo, and do not reintroduce the visual shortcut
indicators.
...
`Capture.host_viewer_mirror_state/0` (reads the mirror slot only while a
`HostLive` is registered via `Capture.track_host/0`; otherwise treats the
viewer as closed / live). The slot itself lives in `Capture.HostViewerState`,
a small `Agent` — not `:persistent_term`, which triggers a global GC on every
write and is unfit for a slot that changes on every Host navigation. No
classification, no Presence, no own navigation: when the Host viewer is
closed or `follow`, `/tv` stays on the latest photo; when the Host browses
with `follow: false`, `/tv` shows that photo. `host_viewer_state/0` is a
single global slot, not per-connection — a second Host tab open at the same
time overwrites it for every `/tv`, so one Host tab at a time is the
supported topology; do not build per-connection state for this. Do not rely
on Host `terminate`/`open: false` to clear the slot — network drop, crash, or
abrupt shutdown never announce; Registry drops the Host on process death and
...
- `Revela.Capture.CameraServer` supervises `gphoto2 --capture-tethered` for tethered shooting. It sends `SIGKILL` (never `SIGTERM`, which gphoto2 ignores) to stop the process; killing it mid-PTP-transfer wedges the Canon camera hard (see `@moduledoc` and `maybe_stop_for_low_disk/1` for the disk-space preventive-stop mechanism, which only acts between shots). Floor defaults to 5 GiB; override with start opt `min_free_disk_bytes` or env `TETHER_MIN_FREE_DISK_BYTES` (documented in README). Auto-arm: with an active editorial, USB camera present, and disk awareness `:available` above the floor, tether starts after a short presence debounce without a click; never into limbo `_sem-editorial`, and never when `disk_awareness` is `:unavailable`. Explicit `stop_capture` sets an `operator_stopped` latch until `start_capture`; reconnect-while-desired is unchanged. Tether spawn failure clears `desired` and arms a short auto-arm cooldown (no phantom reconnect / no tight loop). When the folder watcher cannot start, public status exposes `ingest_awareness: :unavailable` so Host can show degraded tether honestly.
- Demo/test without a camera: `REVELA_DEMO=1` only (env — never compile-time config alone, never a Host toggle). Fake presence, never spawns gphoto2, Host badge + **Disparar (demo)** / `D` (window `D` only after an editorial exists; use the button before that) writes a synthetic JPEG into the editorial folder for the real inotify→ingest path (or settle-direct when the file watcher is unavailable). See README and `CameraServer.demo_fire/1`.
